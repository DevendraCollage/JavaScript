...
//? When a web browser loads an HTML document, it parses the HTML sour ce code and creates a tree-like structure known as the Document Object Model (DOM). This DOM tree represents the structure of the HTML document, with each HTML elements being a node in the tree.
...
// TODO: The Document Object Model (DOM) is an Application Programming Interface (API). The DOM Tree is the structure of your HTML document, as represented by the DOM API. As stated, this API then give us many methods and properties that we can use to manipulate the Tree, and therefore, by extension, the document.
...
//? Comment node: Represents comments within the HTML document.

//? Processing instruction node: A processing instruction node, such as <? xml-stylesheet ...?>.

//? Document node: A document node.

//? Document type node: A document type node, suc as <! DOCTYPE html>.
...
//? parentNode / parentElement
// Navigate to the parent noe of the element.

// Document and DocumentFragment nodes can never have a parent, so parentNode will always return null. It also returns null if the node has ust been created and is not yet attached to the tree.

//? childNodes / children:
// Navigate to the child nodes or elements.

// childNodes is a property that return a NodeLIst containing all child nodes of a given element, including text nodes and comment nodes.
...
//? closest(selector):
//? Find the closest that matches a given selector.
//? The closest(selector) method is used find the closest ancestor of an element that matches a specified CSS selector. This method traverses up the DOM tree, string from the current element, and returns the first ancestor that matches the provided selector. If no matching ancestor is found, it returns null.
...
//? getElementByClassName(className): Find an element with a specified class name.

//? getElementByTagName(tagName): Find an element with specified tag name.

//? querySelector(selector): Find the first element that matches the specified CSS selector.

//? querySelectorAll(selector): Find all elements that mach the specified CSS selector.
